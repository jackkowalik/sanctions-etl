...
class SwissSECO implements SourceInterface
{
    ...
    private const SOURCE_ID = 'ch_seco';
    ...
    public function fetch(?string $lastHash = null): FetchResult
    {
        ...
        $this->logger->info("Fetching Swiss SECO sanctions list", ['url' => self::URL]);
        ...
        if ($ok === false || $errno !== 0 || $httpCode !== 200) {
            @unlink($destFile);
            throw new \RuntimeException("Failed to fetch SECO list: HTTP {$httpCode}, curl errno {$errno} - {$error}");
        }

        $fileSize = filesize($destFile);
        if ($fileSize === 0) {
            @unlink($destFile);
            throw new \RuntimeException("SECO returned empty response");
        }
        ...
        $this->logger->info("SECO data fetched", [
            'http_code' => $httpCode,
            'file_size' => $fileSize,
            'hash' => $hash,
        ]);
